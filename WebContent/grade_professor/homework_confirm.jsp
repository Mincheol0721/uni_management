	                       ...
	                       <table class="pull-right">
							<tr>
								...
								<td>
									...
									<td><a type="button" class="btn btn-primary" href="homework_confirm.jsp?studentName=<%=studentName%>&cname=<%=cname%>">목록으로</a></td>
							</tr>
						</table>
                   	          ...
                   	          <%
              	          		...
              	          		%>
                   	          		...
                   	          		<tr align="center" style="border-bottom: 1px, solid, lightgrey;" id="searchZone">                 	           			
                   	           			...
                   	           			<td width=5%><a href="homeWorkModify_confirm.jsp?num=<%=home.getNum()%>&cname=<%=cname%>" style="text-decoration: none"><%=home.getTitle()%></a></td>
                        ...
                        <%
                        	if(count>0) {
                        		...
                        		if(startPage > pageBlock) {
                        			if(searchText != null & searchText != ""){
                        				%><a href="homework_confirm.jsp?pageNum=<%=startPage-pageBlock%>&studentName=<%=studentName%>&cname=<%=cname%>&searchText=<%=searchText%>&searchField=<%=searchField%>">[이전]</a><%
                        			}else {
                        				%><a href="homework_confirm.jsp?pageNum=<%=startPage-pageBlock%>&studentName=<%=studentName%>&cname=<%=cname%>">[이전]</a><%
                        			}
                        		}
                        		//[1][2][3][4]...[10]
                        		for(int i=startPage; i<=endPage; i++) {
                        			if(searchText != null & searchText != ""){
                        				%><a href="homework_confirm.jsp?pageNum=<%=i%>&cname=<%=cname%>&studentName=<%=studentName%>&searchText=<%=searchText%>&searchField=<%=searchField%>" id="a">[<%=i%>]</a><%
                        			}else {
                        				%><a href="homework_confirm.jsp?pageNum=<%=i%>&studentName=<%=studentName%>&cname=<%=cname%>" id="a">[<%=i%>]</a><%
                        			}
                        		}
                        		//[다음] 끝페이지 번호가 전체페이지수 보다 작을때..
                        		if(endPage < pageCount) {
                        			if(searchText != null & searchText != ""){
                        				%><a href="homework_confirm.jsp?pageNum=<%=startPage+pageBlock%>&studentName=<%=studentName%>&cname=<%=cname%>&searchText=<%=searchText%>&searchField=<%=searchField%>" id="b">[다음]</a><%
                        			}else {
                        				%><a href="homework_confirm.jsp?pageNum=<%=startPage+pageBlock%>&studentName=<%=studentName%>&cname=<%=cname%>" id="b">[다음]</a><%
                        			}
                        		}
                        	}
                        %>
...
//         		alert(cname);
        		...
        		if(searchText != null & searchText != ""){
        			location.href = 'homework_confirm.jsp?pageNum=1&searchText='+ searchText + '&searchField='+ searchField + '&cname=' + cname;
        		}
        		
        		jQuery.ajaxSettings.traditional = true;
    			$.ajax({
    				
    				url : "searchHomeworkBBS2_confirm.jsp", //요청할 서버페이지 경로 
